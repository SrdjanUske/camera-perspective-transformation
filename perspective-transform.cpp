...
extern "C" bool mouse_click_and_param_init(void* init_bgr_frame)
{
    ...
    cv::namedWindow("mouse_input_event", cv::WINDOW_AUTOSIZE);
    cv::imshow("mouse_input_event", *opencv_bgr_frame);
    

    if (!clickEventFinished)
    {
        if(!click_finished)
            cv::setMouseCallback("mouse_input_event", MouseCallBackFunc, (void*)&opencv_bgr_frame);
        else
        {
...
{
    cv::Mat* opencv_bgr_frame = (cv::Mat*)input_bgr_frame;
    ...
    for (int m = 0; m < NUM_KEY_POINTS; m++)
    ...
    {
        cv::destroyWindow("mouse_input_event");
        cv::destroyWindow("output_finetune");
    }
}
#ifdef __cplusplus
}
